...
const JSON_PATH = path.resolve(__dirname, '../src/data/tg_eapcet_2023_allotments.json');

export async function seedTg2023Allotments() {
  ...
  console.log(`Loaded ${Object.keys(collegeMap).length} colleges from TG EAPCET 2023 JSON.`);

  const client = await pool.connect();
  try {
    console.log('Cleaning existing tg-eapcet 2023 allotment records...');
    await client.query("DELETE FROM eapcet_allotment_records WHERE exam_id = 'tg-eapcet' AND admission_year = 2023");

    console.log('Inserting 2023 candidate records into PostgreSQL database...');
    let totalInserted = 0;
    let totalSkipped = 0;
    ...
    for (const cCode of Object.keys(collegeMap)) {
      ...
      for (const branch of (col.branches || [])) {
        ...
        for (let idx = 0; idx < candidates.length; idx++) {
          ...
          const phase = 'final';
          const rollNo = c.rollNo || `TG23-${collegeCode}-${branchCode}-${idx + 1}`;
    ...
    console.log(`\n🎉 Seed 2023 Completed Successfully!`);
