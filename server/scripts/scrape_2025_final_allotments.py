...
print("=== Starting TG EAPCET 2025 Final Phase Data Scraper ===", flush=True)
...
# 2. Fetch colleges
print("Fetching 2025 Final Phase College Directory...", flush=True)
col_html = fetch_with_retry(base_url + "fetch_colleges.php?year=2025_FINAL_PHASE", api_headers)
if not col_html:
    print("Error fetching colleges!", flush=True)
    exit(1)
...
print(f"Found {len(colleges)} colleges for TG EAPCET 2025 Final Phase.", flush=True)

output_file = os.path.join(os.path.dirname(__file__), "../src/data/tg_eapcet_2025_final_allotments.json")
...
for idx, col in enumerate(colleges, 1):
    ...
    time.sleep(0.05)
    b_html = fetch_with_retry(f"{base_url}fetch_branches.php?year=2025_FINAL_PHASE&college={c_code}", api_headers)
    if not b_html:
    ...
    for b in branches:
        ...
        b_name = b['name']
        time.sleep(0.02)
        res_json = fetch_with_retry(f"{base_url}fetch_results.php?year=2025_FINAL_PHASE&college={c_code}&branch={b_code}", api_headers, is_json=True)
        if res_json:
            try:
...
print("==================================================", flush=True)
print(f"Scrape Complete! Total Colleges: {len(all_allotments_by_college)}, Total Candidate Records: {total_candidate_records}", flush=True)
...
print(f"Saved complete dataset to {output_file}", flush=True)
