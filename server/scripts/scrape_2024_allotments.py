...
print("=== Starting TG EAPCET 2024 Data Scraper ===", flush=True)
...
# 2. Fetch colleges
print("Fetching 2024 College Directory...", flush=True)
col_html = fetch_with_retry(base_url + "fetch_colleges.php?year=2024", api_headers)
if not col_html:
    print("Error fetching 2024 colleges!", flush=True)
    exit(1)
...
print(f"Found {len(colleges)} colleges for TG EAPCET 2024.", flush=True)

output_file = os.path.join(os.path.dirname(__file__), "../src/data/tg_eapcet_2024_allotments.json")
...
for idx, col in enumerate(colleges, 1):
    ...
    time.sleep(0.05)
    b_html = fetch_with_retry(f"{base_url}fetch_branches.php?year=2024&college={c_code}", api_headers)
    if not b_html:
    ...
    for b in branches:
        ...
        b_name = b['name']
        time.sleep(0.02)
        res_json = fetch_with_retry(f"{base_url}fetch_results.php?year=2024&college={c_code}&branch={b_code}", api_headers, is_json=True)
        if res_json:
            try:
...
print("==================================================", flush=True)
print(f"Scrape Complete! Total 2024 Colleges: {len(all_allotments_by_college)}, Total Candidate Records: {total_candidate_records}", flush=True)
...
print(f"Saved complete 2024 dataset to {output_file}", flush=True)
