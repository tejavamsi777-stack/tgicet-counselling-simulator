...
const JSON_PATH = path.resolve(__dirname, '../src/data/tg_eapcet_2022_allotments.json');

export async function seedTg2022Allotments() {
  ...
  console.log(`Loaded ${Object.keys(collegeMap).length} colleges from TG EAPCET 2022 JSON.`);

  const client = await pool.connect();
  try {
    console.log('Cleaning existing tg-eapcet 2022 allotment records...');
    await client.query("DELETE FROM eapcet_allotment_records WHERE exam_id = 'tg-eapcet' AND admission_year = 2022");

    console.log('Inserting 2022 candidate records into PostgreSQL database...');
    let totalInserted = 0;
    let totalSkipped = 0;
    ...
    for (const cCode of Object.keys(collegeMap)) {
      ...
      for (const branch of (col.branches || [])) {
        ...
        for (let idx = 0; idx < candidates.length; idx++) {
          ...
          const phase = 'final';
          const rollNo = c.rollNo || `TG22-${collegeCode}-${branchCode}-${idx + 1}`;
    ...
    console.log(`\n🎉 Seed 2022 Completed Successfully!`);
