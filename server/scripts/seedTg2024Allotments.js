...
const JSON_PATH = path.resolve(__dirname, '../src/data/tg_eapcet_2024_allotments.json');

export async function seedTg2024Allotments() {
  ...
  console.log(`Loaded ${Object.keys(collegeMap).length} colleges from TG EAPCET 2024 JSON.`);

  const client = await pool.connect();
  try {
    console.log('Cleaning existing tg-eapcet 2024 allotment records...');
    await client.query("DELETE FROM eapcet_allotment_records WHERE exam_id = 'tg-eapcet' AND admission_year = 2024");

    console.log('Inserting 2024 candidate records into PostgreSQL database...');
    let totalInserted = 0;
    let totalSkipped = 0;
    ...
    for (const cCode of Object.keys(collegeMap)) {
      ...
      for (const branch of (col.branches || [])) {
        ...
        for (let idx = 0; idx < candidates.length; idx++) {
          ...
          const phase = 'final';
          const rollNo = c.rollNo || `TG24-${collegeCode}-${branchCode}-${idx + 1}`;
    ...
    console.log(`\n🎉 Seed 2024 Completed Successfully!`);
