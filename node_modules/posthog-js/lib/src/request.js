...
var jsonStringify = function (data, space) {
    try {
        ...
        return (0, core_1.safeJsonStringify)(data);
    }
};
...
var VERSIONLESS_ENDPOINTS = ['/e/', '/s/'];
var getURLPath = function (url) {
    var parsedURL = (0, request_utils_1.convertToURL)(url);
    var path = (parsedURL === null || parsedURL === void 0 ? void 0 : parsedURL.pathname) || url.split(/[?#]/)[0];
    return path ? (path[0] === '/' ? path : "/".concat(path)) : '/';
};
var hasEndpointSuffix = function (path, endpoint) {
    return path.slice(path.length - endpoint.length) === endpoint;
};
var isVersionlessEndpoint = function (url) {
    var path = getURLPath(url);
    return VERSIONLESS_ENDPOINTS.some(function (endpoint) { return hasEndpointSuffix(path, endpoint); });
};
var buildRequestURL = function (url, compression) {
    var versionlessEndpoint = isVersionlessEndpoint(url);
    var requestURL = versionlessEndpoint ? removeURLParam(url, 'ver') : url;
    return (0, exports.extendURLParams)(compression === types_1.Compression.GZipJS ? removeURLParam(requestURL, 'compression') : requestURL, __assign(__assign({ _: new Date().getTime().toString() }, (versionlessEndpoint ? {} : { ver: config_1.default.JS_SDK_VERSION })), (compression === types_1.Compression.GZipJS ? {} : { compression: compression })));
};
