...
export default function LoginCharacters({ emailFocused, passwordFocused, showPassword, success }) {
  ...
  // Layout offsets per character to create an overlapping cluster like the
  // reference image: tall one sits back/top-center, others layer in front.
  const LAYOUT = {
    tall: { marginLeft: 0, marginBottom: 0, z: 10 },
    medium2: { marginLeft: -18, marginBottom: 6, z: 20 },
    short: { marginLeft: -34, marginBottom: -6, z: 30 },
    medium: { marginLeft: -22, marginBottom: -2, z: 40 },
  };

  return (
    <div className="relative mb-6 flex items-end justify-center">
      {CHARACTER_SPECS.map((spec) => {
        const l = LAYOUT[spec.key];
        return (
          <div
            key={spec.key}
            style={{ marginLeft: l.marginLeft, marginBottom: l.marginBottom, zIndex: l.z, position: "relative" }}
          >
            <Character spec={spec} mouse={mouse} stage={stage} />
          </div>
        );
      })}
