...
export default function PolycetMockCounsellingPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const heroRef = useRef(null);

  const EXAM_SLUG = "tg-polycet";
  const STORAGE_NS = "tgpolycet";
  ...
  useEffect(() => {
    ...
    async function fetchColleges() {
      ...
      try {
        ...
        if (!cancelled) { setCollegesError(err.message || "Failed to load polytechnic colleges."); setAllColleges([]); }
      } finally {
  ...
  function handleCandidateSubmit() {
    if (rank.toString().trim() === "") { setCandidateError("Please enter your TG POLYCET Rank"); return; }
  ...
  if (step === "candidate") {
    ...
    stepContent = (
      <div className="space-y-1.5" style={{ fontFamily: "Arial, Helvetica, sans-serif" }}>
        ...
        <PreferenceList colleges={availableColleges} preferences={preferences} setPreferences={setPreferences} courseGroups={listCourseGroups} />
      </div>
    );
  }
  ...
  return (
    <main className={`relative mx-auto overflow-visible ${step === "list" ? "max-w-[1375px] space-y-1.5 px-0 pb-4 pt-2" : "max-w-6xl space-y-8 px-6 pb-44 pt-6"}`}>
      <Seo
        title="TG POLYCET 2027 Web Options Simulator | Mock Option Entry Practice Portal"
        description="Practice TG POLYCET 2027 diploma web options entry with zero risk. Official portal replica with real college codes, branch priority reordering, and PDF download."
        keywords="tg polycet web options simulator 2027, ts polycet mock counselling portal, tgpolycet.nic.in web options demo practice, polycet choice list generator"
        path="/exams/tg-polycet/mock-counselling"
        toolType="simulator"
        examName="TG POLYCET"
      />
      ...
      <div className="w-full rounded-xl border border-red-500/40 bg-red-950/40 p-2.5 sm:px-4 sm:py-2 text-center shadow-lg backdrop-blur-md">
        <p className="text-xs sm:text-sm font-bold text-red-300 tracking-wide flex items-center justify-center gap-1.5">
          ...
          <span>
            <strong>NOTE:</strong> This is a <strong>Mock Web Options Simulator</strong> for candidate practice and preference ordering only. This is <u>NOT</u> the official TGCHE / SBTET website. Submit your final web options on <strong>tgpolycet.nic.in</strong>.
          </span>
        </p>
      </div>
      ...
      <div ref={heroRef}>
        <GlowCard customSize={true} glowColor="purple" className="p-8 sm:p-12 text-center">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/10 px-3.5 py-1 text-xs font-semibold uppercase tracking-wider text-purple-300 backdrop-blur-sm">
            <Sparkles size={12} />
            <span>TG POLYCET Practice Web Options</span>
          </span>
          <h1 className="mt-4 text-4xl font-bold tracking-tight text-white sm:text-5xl" style={{ fontFamily: "var(--font-display)" }}>
            <ScrambleText text="POLYCET Mock Counselling" duration={800} /> Simulator
          </h1>
          <p className="mt-3 text-base text-gray-300">
            TG POLYCET (Polytechnic &amp; Diploma) — Select your district(s), then build your preference list.
          </p>
        </GlowCard>
      </div>
      ...
      <ToolGuideSection toolType="mock_counselling" examName="TG POLYCET" authorityName="SBTET Telangana" />
    </main>
  );
}
