...
export default function EapcetMockCounsellingPage() {
  const navigate = useNavigate();
  const heroRef = useRef(null);

  const EXAM_SLUG = "ap-eapcet";
  const STORAGE_NS = "tgeapcet";
  ...
  function handleCandidateSubmit() {
    if (rank.toString().trim() === "") { setCandidateError("Please enter your AP EAPCET Rank"); return; }
      ...
      <div ref={heroRef}>
        <GlowCard customSize={true} glowColor="purple" className="p-8 sm:p-12 text-center">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/10 px-3.5 py-1 text-xs font-semibold uppercase tracking-wider text-purple-300 backdrop-blur-sm">
            <Sparkles size={12} />
            <span>AP EAPCET Practice Web Options</span>
          </span>
          ...
          <p className="mt-3 text-base text-gray-300">
            AP EAPCET (Engineering / Pharmacy) — Select your district(s), then build your preference list.
          </p>
        </GlowCard>
      </div>
