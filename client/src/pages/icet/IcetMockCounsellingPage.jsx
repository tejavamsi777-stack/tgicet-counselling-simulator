...
export default function IcetMockCounsellingPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const heroRef = useRef(null);

  const EXAM_SLUG = "tg-icet";
  const STORAGE_NS = "tgicet";
  ...
  function handleCandidateSubmit() {
    if (rank.toString().trim() === "") { setCandidateError("Please enter your TG ICET Rank"); return; }
  ...
  const listCourseGroups = [
    { title: "GROUP MBA — MBA / MBT / MTM", courses: ["MBA", "MBT", "MTM"] },
    { title: "GROUP MCA — MCA", courses: ["MCA"] },
  ];
      ...
      <div className="space-y-6">
        {collegesLoading ? (
          ...
          <GlowCard customSize={true} glowColor="purple" className="p-6 sm:p-8">
            <DistrictSelector districts={availableDistricts} selectedDistricts={selectedDistricts} setSelectedDistricts={setSelectedDistricts} error={districtError} courseGroups={ICET_COURSE_GROUPS} />
          </GlowCard>
        )}
    ...
    <main className={`relative mx-auto overflow-visible ${step === "list" ? "max-w-[1375px] space-y-1.5 px-0 pb-4 pt-2" : "max-w-6xl space-y-8 px-6 pb-44 pt-6"}`}>
      ...
      <div className="w-full rounded-xl border border-red-500/40 bg-red-950/40 p-2.5 sm:px-4 sm:py-2 text-center shadow-lg backdrop-blur-md">
        <p className="text-xs sm:text-sm font-bold text-red-300 tracking-wide flex items-center justify-center gap-1.5">
          ...
          <span>
            <strong>NOTE:</strong> This is a <strong>Mock Web Options Simulator</strong> for candidate practice and preference ordering only. This is <u>NOT</u> the official TGCHE website. Submit your final web options on <strong>tgicet.nic.in</strong>.
          </span>
        </p>
      </div>
