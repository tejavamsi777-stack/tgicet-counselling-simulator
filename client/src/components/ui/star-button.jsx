...
function StarBackground({ bgColor = "#0f172a", starColor = "#ffffff" }) {
  return (
    <svg
      width="100%"
      height="100%"
...
export function StarButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "group/star-button relative z-[3] inline-flex h-10 items-center justify-center gap-2 overflow-hidden rounded-3xl px-5 py-2 text-sm font-bold whitespace-nowrap transition-all disabled:pointer-events-none disabled:opacity-50 cursor-pointer shadow-lg",
        className
      )}
      {...props}
    >
      {/* Animated Glowing Light Edge */}
      <div
        className="animate-star-btn absolute inset-0 aspect-square bg-[radial-gradient(ellipse_at_center,var(--light-color),transparent,transparent)] pointer-events-none z-[2]"
        style={{
          offsetPath: "var(--path)",
          offsetDistance: "0%",
      ...
      {/* Background Star Dots Canvas */}
      <div
        className="absolute inset-0 z-[1] overflow-hidden rounded-[inherit] border-white/20 pointer-events-none"
        style={{ borderWidth: "var(--border-width)" }}
        aria-hidden="true"
      >
        <StarBackground bgColor={backgroundColor} starColor={starColor} />
      </div>

      {/* Button Content - Guaranteed High Contrast & 100% Readable */}
      <span className="relative z-10 inline-flex items-center gap-1.5 font-black text-white drop-shadow-sm">
        {children}
      </span>
    </button>
  )
