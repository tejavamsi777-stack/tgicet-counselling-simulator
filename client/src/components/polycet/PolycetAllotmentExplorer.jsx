...
export function getSpecialCategoryType(candidate) {
  ...
  const sp = String(candidate.specialCategory || candidate.caste || candidate.category || '').toUpperCase();
...
function AnimatedCounter({ value, duration = 800, prefix = '', suffix = '', isVisible = true }) {
  ...
  }, [value, duration, isVisible, isBlank]);
...
function InteractiveCategoryChart({ candidates = [] }) {
  ...
  // 3. Sub-quota Allotment Categories
...
export default function PolycetAllotmentExplorer({ onDataLoaded }) {
  ...
  const categoryAnalyticsData = useMemo(() => {
    ...
    if (analyticsCategory !== 'ALL') {
      if (analyticsCategory === 'SPECIAL') {
        pool = candidatesList.filter((c) => isSpecialCategory(c));
      } else if (analyticsCategory === 'OC') {
        pool = candidatesList.filter((c) => {
          if (isSpecialCategory(c)) return false;
          const rawSeat = (c.seatCategory || '').toUpperCase();
          const rawCaste = (c.caste || '').toUpperCase();
          if (rawSeat.includes('EWS') || rawCaste.includes('EWS')) return false;
    ...
    if (!pool.length) {
      return {
        totalCandidates: 0,
        specialIgnoredCount: candidatesList.filter((c) => isSpecialCategory(c)).length,
        openingRank: null,
        closingRank: null,
        boyCount: 0,
        girlCount: 0,
  ...
  }, [candidatesList, analyticsCategory]);
      ...
      <div className="pt-6 border-t border-white/[0.08] space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <InteractiveGenderChart
            candidates={categoryAnalyticsData?.candidates || candidatesList}
        ...
        <RankDistributionHistogram candidates={categoryAnalyticsData?.candidates || candidatesList} />
      </div>
    </div>
  )}
