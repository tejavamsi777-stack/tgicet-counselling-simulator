...
function InteractiveCategoryChart({ candidates = [] }) {
  ...
  const groupedData = useMemo(() => {
    ...
    candidates.forEach((c) => {
      const rootCat = (c.caste || c.seatCategory?.split('_')[0] || 'OC').toUpperCase();
      if (!map[rootCat]) {
        map[rootCat] = {
          name: rootCat,
          count: 0,
  ...
  {!hasQueried && !fetching && (
    <div className="rounded-3xl border border-white/[0.06] bg-black/30 py-20 text-center backdrop-blur-xl">
      ...
      <p className="text-sm text-white/20 mt-1">
        then click <span className="text-purple-400 font-bold">Show Allotment Data</span> to view official candidate records &amp; visual analytics
      </p>
    </div>
  )}
