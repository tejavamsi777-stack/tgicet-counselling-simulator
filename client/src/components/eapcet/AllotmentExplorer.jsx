...
export default function AllotmentExplorer() {
  // Meta (years, colleges, branches)
  const [meta, setMeta] = useState({ years: [], colleges: [], branches: [] });
  ...
  const currentBranchObj = useMemo(() => {
    return meta.branches.find((b) => b.code === selectedBranch) || null;
  }, [meta.branches, selectedBranch]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
          ...
          <div>
            <label className="text-xs font-bold uppercase tracking-wider text-white/60 mb-2 flex items-center gap-1.5">
              ...
              Branch / Programme Stream ({meta.branches.length})
            </label>
            <SearchableSelect
              ...
              placeholder="-- Search / Select Branch Stream --"
              searchPlaceholder="Search branch code (CSE, ECE) or name..."
              options={meta.branches.map((b) => ({
                value: b.code,
                label: `${b.code} — ${b.name}`,
              }))}
            />
          ...
          <p className="text-xs text-white/50">
            {currentCol && currentBranchObj ? (
              <>
                ...
                <span className="text-cyan-300">{currentBranchObj.code}</span>
              </>
            ) : (
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-xs text-white/50">• Engineering College</span>
                  <span className="text-xs text-cyan-300">• Verified TSCHE Portal</span>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-white/60">
                  <span className="inline-flex items-center gap-1">
                    ...
                    Branch: <b className="text-white">{results.branch?.name || selectedBranch} ({selectedBranch})</b>
                  </span>
                  <span>
                    • Status: <b className="text-emerald-400">✓ Official Convener Allotment Data</b>
                  </span>
                </div>
              </div>
      ...
      {!hasQueried && !fetching && (
        <div className="rounded-3xl border border-white/[0.06] bg-black/30 py-20 text-center backdrop-blur-xl">
          ...
          <p className="text-sm text-white/20 mt-1">
            then click <span className="text-purple-400 font-bold">Show Allotment Data</span> to view official TSCHE candidate records &amp; visual analytics
          </p>
        </div>
      )}
    </div>
