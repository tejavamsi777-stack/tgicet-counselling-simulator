...
const SPECIAL_CATS = ['ph', 'cap', 'ncc', 'sports', 'special quota'];
    ...
    case 'oc':
      return documents.filter(
        (d) =>
          ...
          d.applicableFor?.some((a) => a.toLowerCase() === 'all')
      );
    case 'ews':
      return documents.filter(
        (d) =>
          d.categories?.includes('ews') ||
          d.categories?.includes('all') ||
          d.applicableFor?.some((a) => a.toLowerCase() === 'ews' || a.toLowerCase() === 'all')
      );
    case 'bc':
      return documents.filter(
        (d) =>
          d.categories?.includes('bc') ||
          d.categories?.includes('all') ||
          d.applicableFor?.some((a) => a.toLowerCase() === 'bc' || a.toLowerCase() === 'all')
      );
    case 'sc_st':
      return documents.filter(
        (d) =>
          ...
          d.categories?.includes('all') ||
          d.applicableFor?.some(
            (a) => a.toLowerCase() === 'sc/st' || a.toLowerCase() === 'sc' || a.toLowerCase() === 'st' || a.toLowerCase() === 'all'
          )
      );
    case 'minority':
      return documents.filter(
        (d) =>
          d.categories?.includes('minority') ||
          d.categories?.includes('all') ||
          d.applicableFor?.some((a) => a.toLowerCase() === 'minority' || a.toLowerCase() === 'all')
      );
    case 'special':
      return documents.filter(
        (d) =>
          d.categories?.some((c) => SPECIAL_CATS.includes(c.toLowerCase())) ||
          d.applicableFor?.some((a) => SPECIAL_CATS.includes(a.toLowerCase()))
      );
    default:
      return documents;
  }
...
export default function CertificateChecklist({ documents = [], examSlug = 'tg-ecet' }) {
    ...
    <div className="rounded-2xl border border-white/[0.08] bg-black/40 backdrop-blur-xl p-6">
      ...
      <div className="no-print mb-5 flex justify-end">
        <GlassButton
          ...
          contentClassName="flex items-center gap-1.5 cursor-pointer"
        >
          <Download size={13} />
      ...
      <div className="overflow-x-auto rounded-xl border border-white/[0.06] bg-white/[0.01]">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-white/[0.04]">
            {filtered.map((doc, i) => {
              const isChecked = Boolean(ticked?.has(doc.id));
              return (
                <tr
                  key={doc.id}
                ...
                >
                  ...
                  <td className="py-3.5 px-4">
                    <div
                      className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all duration-200 ${
                        isChecked ? 'bg-purple-500 border-purple-400' : 'border-white/20 bg-transparent'
                      }`}
                    >
                  ...
                  <td className="py-3.5 px-4">
                    ...
                    {(doc.desc || doc.purpose || doc.note) && (
                      <p className="text-white/40 text-xs mt-0.5">{doc.desc || doc.purpose || doc.note}</p>
                    )}
                    {doc.xeroxSets && (
                      <p className="text-purple-300/60 text-[11px] mt-0.5">
                        📋 {doc.xeroxSets} photocopy set{doc.xeroxSets > 1 ? 's' : ''} required
                      </p>
                    )}
                  </td>
                  <td className="py-3.5 px-4 hidden sm:table-cell">
                    {(doc.validity || doc.desc) && (
                      <span className="text-amber-300/80 text-xs">{doc.validity || doc.desc}</span>
                    )}
                  </td>
                  <td className="py-3.5 px-4">
                    {isChecked ? (
