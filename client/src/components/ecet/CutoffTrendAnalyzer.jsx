...
const BRANCHES = [
  ...
  { code: 'CSD', label: 'Data Science' },
  { code: 'INF', label: 'IT' },
  { code: 'ECE', label: 'ECE' },
  { code: 'EEE', label: 'EEE' },
  { code: 'MEC', label: 'Mechanical' },
  { code: 'CIV', label: 'Civil' },
];

export default function CutoffTrendAnalyzer() {
  const [branch, setBranch] = useState('CSE');
  const [colleges, setColleges] = useState(ECET_INSTITUTIONS.slice(0, 6));
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    ecetApi
      .getColleges({ branch, sort: 'rank' })
      .then((res) => {
        const list = res?.data || (Array.isArray(res) ? res : []);
        if (list.length > 0) setColleges(list.slice(0, 6));
      })
      .catch(console.error)
    ...
    <div className="relative overflow-hidden rounded-2xl border border-white/[0.08] bg-black/40 backdrop-blur-xl p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-xl font-bold text-white">Lateral Entry Cutoff Trajectory &amp; Shifts (2024–2025)</h3>
            <p className="text-xs text-white/50">Tracking closing rank movement and competition across diploma-to-degree admissions</p>
          </div>
        </div>
          ...
          <table className="w-full text-xs sm:text-sm">
            <thead>
              <tr className="border-b border-white/[0.08] text-white/40 bg-white/[0.02]">
                ...
                <th className="py-3.5 px-4 sm:px-6 text-center font-semibold text-purple-300">2025 Cutoff</th>
                <th className="py-3.5 px-4 sm:px-6 text-center font-semibold">BC-B Cutoff</th>
                <th className="py-3.5 px-4 sm:px-6 text-center font-semibold">SC Cutoff</th>
            ...
            <tbody className="divide-y divide-white/[0.04]">
              {colleges.map((c) => {
                ...
                if (!info) return null;
                const r24 = info.oc2024 || 0;
                const r25 = info.oc2025 || 0;
                const isTougher = r25 < r24 && r24 > 0;
                const diff = Math.abs(r24 - r25);
                ...
                return (
                  <tr key={c.code} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="py-3.5 px-4 sm:px-6 text-center font-mono text-white/70">
                    ...
                    <td className="py-3.5 px-4 sm:px-6 text-center font-mono text-amber-300">
                      {info.bcb2025 ? `#${info.bcb2025.toLocaleString()}` : '—'}
                    </td>
                    <td className="py-3.5 px-4 sm:px-6 text-center font-mono text-purple-300">
                      {info.sc2025 ? `#${info.sc2025.toLocaleString()}` : '—'}
                    </td>
