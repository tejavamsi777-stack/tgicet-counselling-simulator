...
/**
 * Parses date strings like "August 06 – August 10, 2026" or "August 12, 2026"
...
function isPhaseCurrentlyLive(phase) {
  ...
  const events = phase.steps || phase.events || phase.schedule || [];
  if (events.length > 0) {
...
export default function PhaseScheduleTable({ phases = [] }) {
  ...
  const defaultPhaseId = useMemo(() => {
    ...
    const upcomingPhase = phases.find(p => {
      const events = p.steps || p.events || p.schedule || [];
